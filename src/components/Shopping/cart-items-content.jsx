...
function UserCartItemsContent({ cartItem }) {
    ...
    function handleUpdateQuantity(getCartItem, typeOfAction) {
        if (typeOfAction == 'plus') {
            ...
            if (getCartItems.length) {
                ...
                console.log(getCurrentProductIndex, getTotalStock, 'getCurrentProductIndex, getTotalStockgetCurrentProductIndex, getTotalStock')
        ...
        dispatch(updateCartQuantity({
            ...
            if (data?.payload?.success) {
                toast({ title: 'cart item  update success' })
            }
        })
    }
    // function handleCartItemDelete(getCartItem) {
    ...
    //         .then(data => {
    //             if (data?.payload?.success) {
    //                 toast({ title: 'cart item delete success' })
    //             }
    //         })
    // }
    ...
    return (
        <div className='flex items-center space-x-4 text-gray-300'>
            <img src={cartItem?.image} alt={cartItem?.title}
                className='w-20 h-20 rounded object-cover'></img>
