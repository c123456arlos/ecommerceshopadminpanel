            ...
            <div onClick={() => handleGetProductDetails(product?._id)}>
                <div className="relative">
                    <img src={product?.image} alt={product?.title}
                        className="w-full h-[300px] object-cover rounded-t-lg"></img>
                    {
                        product?.totalStock === 0 ? <Badge className='absolute top-2 left-2 bg-red-500 hover:bg-red-600'>out of stock</Badge> : product?.totalStock < 10 ? <Badge className='absolute top-2 left-2 bg-red-500 hover:bg-red-600'>{`only ${product?.totalStock}`}</Badge> :
